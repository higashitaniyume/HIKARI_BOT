...
async def _handle_group_msg(
    event: GroupMessageEvent, now: datetime, timestamp: float,
    message_text: str, message_id: int | None,
) -> None:
    qq = event.user_id
    ...
    record = {
        ...
        "sender": {"user_id": qq, "nickname": nickname, "card": card, "role": role},
        "group_id": group_id,
    ...
    logger.info(f"[群消息] 群{group_id} | {qq}({display_name}) | {message_text[:100]}")


async def _handle_private_msg(
    event: PrivateMessageEvent, now: datetime, timestamp: float,
    message_text: str, message_id: int | None,
) -> None:
    qq = event.user_id
    nickname = event.sender.nickname or ""
    ...
    logger.info(f"[私聊消息] {qq}({nickname}) | {message_text[:100]}")
